...
type TariffDtoInput = TariffDto | TariffDto[] | null | undefined

const mapTariffDtoToModel = (rawInput: TariffDtoInput): Tariff | null => {
...
export const getActiveTariff = async (type: VehicleType): Promise<Tariff | null> => {
  try {
    const response = await apiClient.get<TariffDtoInput>('/v1/tariffs/active', {
      params: { type },
      skipToast: true,
    })
